...
hem to be on GPU p_rate (float): attention dropout rate
    ...
    def forward_features(self, x):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in [3,6,9]:
                with torch.no_grad():
                    action_n, action_prob_n = self.agent_n.choose_ation(x.detach(),evaluate=False)
                    # action, action_prob = self.agent.select_action_batch(x.detach())

                mask = torch.where(mask==0, mask, action_n)
                mask[:, 0] = 1
                
                out = torch.unique(mask, return_counts=True)
                 
                if train_agent:
                    self.buffer["state_n"].append(x.detach())
                    self.buffer["action_n"].append(action.detach())
                    self.buffer["action_prob_n"].append(action_prob.detach())
                    self.buffer["mask"].append(mask.detach())
                    x = block.forward(x, mask)
                    self.buffer["state_n_next"].append(x.detach())
                if not train_agent:
                    x = block.forward(x, mask)
